...
use crate::parse_output::run_parse_output;
use crate::paths::LLM_CACHE_DB;
use crate::progress::{ExampleProgress, Step};
...
+++++++ Contents of side #1
    ...
    // Require prompt_inputs to be present
    if example.prompt_inputs.is_none() {
        anyhow::bail!("prompt_inputs missing (run context retrieval first)");
    }

    // Ensure actual_patch is populated (idempotent operation)
    run_parse_output(example).context("Failed to parse prediction output")?;
    ...
    let model = model_for_backend(args.backend);
     ...
     let response = match args.backend {
        BatchProvider::Anthropic => {
            ...
            let Some(response) = client.generate(model, 1024, messages, None).await? else {
                return Ok(());
            };
